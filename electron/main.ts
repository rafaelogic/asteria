...
import {
  ...
  getFile, getGitHubToken, getTree, listBranches, listChecks, listCommits, listIssues, listPullRequests, listRepositories, listReviews, mergePullRequest,
...
async function handleRepairTerminal(sessionId: string, context: NonNullable<ReturnType<typeof sessionContext.get>>, event: { type: string; detail: string }) {
  ...
  if (context.kind === "repair") {
    ...
    try {
      ...
      const promotion = project.radio.autoPushStaging ? await promoteFastForwardToStaging(app.getPath("userData"), project.id, project.repositoryPath!, committed.commit, getGitHubToken()) : { branch: "staging" as const, commit: committed.commit, fastForwardOnly: true as const };
  ...
  if (context.maintenanceGoalId && context.worktreePath && context.sourceRepositoryPath) {
    ...
    if (goal) {
      ...
      try {
        ...
        if (!token) throw new Error("Connect GitHub in Relay before RaDio pushes a verified checkpoint to staging.");
        const promotion = await promoteFastForwardToStaging(app.getPath("userData"), "application", context.sourceRepositoryPath, saved.commit, token);
...
ipcMain.handle("repositories:clone", async (_event, raw) => {
  ...
  return cloneRepository(app.getPath("userData"), input.cloneUrl, input.projectName, input.storagePath, getGitHubToken());
});
