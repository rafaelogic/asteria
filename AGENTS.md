...
Neural Console section controls remain floating around the RaDio core. Start, pause/resume, and stop are the controls centered with the core; do not replace the floating section controls with a dock. Status atmospheres use a restrained neural double-heartbeat language rather than drifting blurred color blobs.
...
Codex-backed RaDio and Star execution routes use the current Sol model across fast, balanced, and frontier agent tiers. Local prompt enhancement is the explicit exception and uses the lower-cost Luna model; it must preserve the owner’s intent and return an editable prompt rather than submitting automatically.
