    ...
    <main ref={stageRef} className="neural-stage" onClick={handleStageClick}>
      ...
      <div className="neural-command-status" role="status" aria-label={`RaDio ${operationalState}`}>
        ...
        <div className="neural-transport-controls" aria-label="RaDio controls">
          <button className="neural-activate-control" aria-label={isWorking ? "Running inspection" : state?.automation.lastCycleAt ? "Inspect now" : "Activate"} title={isWorking ? "Running inspection" : state?.automation.lastCycleAt ? "Inspect now" : "Activate"} disabled={!state || isWorking || state.automation.cycleRunning || state.automation.paused || state.automation.emergencyStopped} onClick={() => void control("run")}><PlayIcon /><span>{isWorking ? "Running" : state?.automation.lastCycleAt ? "Inspect now" : "Activate"}</span></button>
