...
export function FloatingRaDio({ project, onProject, onMaximize }: {
  ...
  const send = async () => {
    if (!body.trim() || !window.asteria || !readiness.ready) return;
    setBusy(true);
    setError("");
    try {
  ...
  return <div className={`floating-radio ${open ? "open" : "minimized"}`}>
    {open && <section className="floating-radio-panel" aria-label="RaDio quick chat">
      ...
      {!readiness.ready && <div className="radio-readiness compact"><strong>{readiness.loading ? "Checking RaDio…" : "RaDio needs setup"}</strong>{readiness.checks.filter((check) => !check.ready).map((check) => <p key={check.label}>{check.label}: {check.detail}</p>)}<button onClick={() => void readiness.refresh()}>Check again</button></div>}
      {error && <p className="radio-send-error" role="alert">{error}</p>}
      <footer><textarea aria-label="Quick message to RaDio" disabled={!readiness.ready} value={body} onChange={(event) => setBody(event.target.value)} placeholder={readiness.ready ? "Message RaDio…" : "Complete RaDio setup first"} onKeyDown={(event) => { if (event.key === "Enter" && !event.metaKey && !event.ctrlKey) { event.preventDefault(); void send(); } }} /><button aria-label="Send quick message" disabled={busy || !body.trim() || !readiness.ready} onClick={() => void send()}><ArrowUpIcon weight="bold" /></button></footer>
    </section>}
