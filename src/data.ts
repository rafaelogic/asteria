...
function project(input: Partial<Project> & Pick<Project, "id" | "name" | "repository" | "objective" | "provider" | "runId">): Project {
  return {
    ...
    skillExecutions: [],
    incidents: [],
    takeover: { projectId: input.id, runId: input.runId, enabled: false, phase: "idle", health: "healthy", updatedAt: now },
    radioChats: [{ id: `chat_${input.runId}`, projectId: input.id, runId: input.runId, archived: false, messages: [], createdAt: now, updatedAt: now }],
